...
output_base_path = "/data/yangchangfan/DiffusionPDE/data/NS_heat-merged/merge_{}.npy"
...
for i in range(number):
    # Q_heat 
    path_Q_heat = os.path.join(f"/data/yangchangfan/DiffusionPDE/data/training/NS_heat/Q_heat/", f'{i+1}.mat')
    Q_heat = sio.loadmat(path_Q_heat)['export_Q_heat']
    Q_heat_normalized = (Q_heat - min_Q_heat) / (max_Q_heat - min_Q_heat) * 1.8 - 0.9 # [-0.9,0.9]

    # u_u
    path_u_u = os.path.join(f"/data/yangchangfan/DiffusionPDE/data/training/NS_heat/u_u/", f'{i+1}.mat')
    u_u = sio.loadmat(path_u_u)['export_u_u']
    u_u_normalized = (u_u - min_u_u) / (max_u_u - min_u_u) * 1.8 - 0.9 # [-0.9,0.9]

    # u_v
    path_u_v = os.path.join(f"/data/yangchangfan/DiffusionPDE/data/training/NS_heat/u_v/", f'{i+1}.mat')
    u_v = sio.loadmat(path_u_v)['export_u_v']
    u_v_normalized = (u_v - min_u_v) / (max_u_v - min_u_v) * 1.8 - 0.9 # [-0.9,0.9]

    # T 
    path_T = os.path.join(f"/data/yangchangfan/DiffusionPDE/data/training/NS_heat/T/", f'{i+1}.mat')
