...
output_base_path = "/data/yangchangfan/DiffusionPDE/data/VA_wrong-merged/merge_{}.npy"
os.makedirs(os.path.dirname(output_base_path), exist_ok=True)

variables = ['p_t', 'Sxx', 'Sxy', 'Syy', 'x_u', 'x_v']

variables_1 = ['p_t', 'Sxx', 'Sxy', 'Syy']
variables_2 = ['x_u', 'x_v']
...
for i in range(number):
    ...
    for var in variables_2:
        data = sio.loadmat(f"{base_path}/{var}/{i+1}.mat")[f"export_{var}"]
        components.append(normalize(data.real/(omega**2 * rho_AL), range_data[var]['min_real'], range_data[var]['max_real']))
        components.append(normalize(data.imag/(omega**2 * rho_AL), range_data[var]['min_imag'], range_data[var]['max_imag']))
