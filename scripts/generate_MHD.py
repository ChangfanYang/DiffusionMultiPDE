...
def generate_MHD(config, start, end):
    ...
    for cur_idx in range(start,end+1):
        print(cur_idx,'----')
        generate_single_MHD(config, cur_idx, net)


def generate_single_MHD(config, offset, net):
    ...
    # scale zeta
        ...
        scale_factor = 0.05 / norm_Br
        zeta_obs_Br = zeta_obs_Br * scale_factor

        if i <= 0.5 * num_steps:
        # if i <= 1 * num_steps:    
            # x_next = x_next - zeta_obs_Br * grad_x_cur_obs_Br - zeta_obs_Jx * grad_x_cur_obs_Jx - zeta_obs_Jy * grad_x_cur_obs_Jy - zeta_obs_Jz * grad_x_cur_obs_Jz - zeta_obs_u_u * grad_x_cur_obs_u_u - zeta_obs_u_v * grad_x_cur_obs_u_v
    ...
    scipy.io.savemat(f'/home/yangchangfan/CODE/DiffusionPDE/MHD_result/MHD_results_{offset}.mat', {'Br': Br_final, 'Jx': Jx_final, 'Jy': Jy_final, 'Jz': Jz_final, 'u_u': u_u_final, 'u_v': u_v_final})
    print('Done.')

def generate_MHD_copy(config):
    """Generate MHD equation."""
    ############################ Load data and network ############################
    datapath = config['data']['datapath']
    ...
    # scale zeta
        ...
        # norm_Jx = torch.norm(zeta_obs_Jx * grad_x_cur_obs_Jx)
        # scale_factor = 1.0 / norm_Jx
        # zeta_obs_Jx = zeta_obs_Jx * scale_factor

        # norm_Jy = torch.norm(zeta_obs_Jy * grad_x_cur_obs_Jy)
        # scale_factor = 1.0 / norm_Jy
        # zeta_obs_Jy = zeta_obs_Jy * scale_factor

        # norm_Jz = torch.norm(zeta_obs_Jz * grad_x_cur_obs_Jz)
        # scale_factor = 1.0 / norm_Jz
        # zeta_obs_Jz = zeta_obs_Jz * scale_factor
        ...
        if i <= 0.5 * num_steps:
            ...
            # norm_value = torch.norm(zeta_obs_Br * grad_x_cur_obs_Br).item()
            # print(norm_value)
            ...
            # x_next = x_next - 0.8*(zeta_obs_Br * grad_x_cur_obs_Br + zeta_obs_Jx * grad_x_cur_obs_Jx + zeta_obs_Jy * grad_x_cur_obs_Jy + zeta_obs_Jz * grad_x_cur_obs_Jz + zeta_obs_u_u * grad_x_cur_obs_u_u + zeta_obs_u_v * grad_x_cur_obs_u_v ) - 0.2* (zeta_pde_NS * grad_x_cur_pde_NS + zeta_pde_J * grad_x_cur_pde_J)
