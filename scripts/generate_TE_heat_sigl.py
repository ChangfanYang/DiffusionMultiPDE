...
def get_TE_heat_loss_sigl(mater, T, mater_GT, T_GT, mater_mask, T_mask, mater_iden, device=torch.device('cuda')):
    ...
    return pde_loss_T, observation_loss_mater, observation_loss_T




def generate_TE_heat_sigl(config, start, end):
...
def generate_single_TE_heat(config, offset, net):
    ...
    for i, (sigma_t_cur, sigma_t_next) in tqdm.tqdm(list(enumerate(zip(sigma_t_steps[:-1], sigma_t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        pde_loss_T, observation_loss_mater, observation_loss_T = get_TE_heat_loss_sigl(mater_N, T_N, mater_GT, T_GT, known_index_mater, known_index_T, mater_iden, device=device)
    ...
    # scale zeta
        ...
        # if i <= 1 * num_steps:
            ...
            x_next = x_next - zeta_pde_T * grad_x_cur_pde_T
    ...
    scipy.io.savemat(f'/home/yangchangfan/CODE/DiffusionPDE/TE_heat_result/TE_heat_results_{offset}.mat', {'mater': mater_final, 'T': T_final})
    print('Done.')
