...
def generate_NS_heat_ycf(config):
...
def generate_NS_heat(config, start, end):
    network_pkl = config['test']['pre-trained']
    print(f'Loading networks from "{network_pkl}"...')
    f = open(network_pkl, 'rb')
    device = config['generate']['device']
    ...
    for cur_idx in range(start,end+1):
        print(cur_idx,'----')
        generate_single_NS_heat(config, cur_idx, net)

def generate_single_NS_heat(config, offset, net):
    datapath = config['data']['datapath']
    device = config['generate']['device']
    ...
    scipy.io.savemat(f'/home/yangchangfan/CODE/DiffusionPDE/NS_heat_result/NS_heat_results_{offset}.mat', {'Q_heat': Q_heat_final, 'u_u': u_u_final, 'u_v': u_v_final, 'T': T_final})
    print('Done.')
