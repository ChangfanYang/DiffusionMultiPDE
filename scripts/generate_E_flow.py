...
def generate_E_flow(config, start, end):
    ...
    for cur_idx in range(start,end+1):
        print(cur_idx,'----')
        generate_single_E_flow(config, cur_idx, net)

def generate_single_E_flow(config, offset, net):
    ...
    scipy.io.savemat(f'/home/yangchangfan/CODE/DiffusionPDE/E_flow_result/E_flow_results_{offset}.mat', {'kappa': kappa_final, 'ec_V': ec_V_final, 'u_flow': u_flow_final, 'v_flow': v_flow_final})
    print('Done.')


def generate_E_flow_copy(config):
    """Generate E_flow equation."""
    ############################ Load data and network ############################
    datapath = config['data']['datapath']
