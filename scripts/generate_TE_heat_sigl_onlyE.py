...
def get_TE_heat_loss(mater, Ez, mater_GT, Ez_GT, mater_mask, Ez_mask, mater_iden, device=torch.device('cuda')):
    """Return the loss of the TE_heat equation and the observation loss."""

    T = 300 * torch.ones(128, device=device)
    ...
    return pde_loss_E, observation_loss_mater, observation_loss_Ez





def generate_TE_heat_sigl_onlyE(config, start, end):
...
def generate_single_TE_heat(config, offset, net):
    ...
    for i, (sigma_t_cur, sigma_t_next) in tqdm.tqdm(list(enumerate(zip(sigma_t_steps[:-1], sigma_t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        pde_loss_E, observation_loss_mater, observation_loss_Ez = get_TE_heat_loss(mater_N, complex_Ez_N, mater_GT, Ez_GT, known_index_mater, known_index_Ez, mater_iden, device=device)
    ...
    # scale zeta
        ...
        # if i <= 1 * num_steps:
            ...
            x_next = x_next - (zeta_pde_E * grad_x_cur_pde_E )
    ...
    scipy.io.savemat(f'/home/yangchangfan/CODE/DiffusionPDE/TE_heat_result/TE_heat_results_{offset}.mat', {'mater': mater_final, 'Ez': complex_Ez_final})
    print('Done.')
