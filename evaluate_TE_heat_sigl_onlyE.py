...
def evaluate(result, GT_data):
    """
    输入:
        result: dict，预测结果，包含 'mater', 'Ez_real', 'Ez_imag'
        GT_data: dict，GT数据，包含 'mater_GT', 'real_Ez_GT', 'imag_Ez_GT'
    输出:
...
# 初始化存储所有结果的字典
all_results = {
    'RMSE': {'mater': [], 'real_Ez': [], 'imag_Ez': []},
    'nRMSE': {'mater': [], 'real_Ez': [], 'imag_Ez': []},
    'MaxError': {'mater': [], 'real_Ez': [], 'imag_Ez': []},
    'bRMSE': {'mater': [], 'real_Ez': [], 'imag_Ez': []},
...
for idx in range(offset_range[0], offset_range[1]):
    try:
        ...
        for metric in ['RMSE', 'nRMSE', 'MaxError', 'bRMSE']:
            for var in ['mater', 'real_Ez', 'imag_Ez']:
